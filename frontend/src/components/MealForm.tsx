...
import { type Meal, type Recipe, mealService, participantMealService, recipeService, ingredientService } from '../services/api';
...
const MealForm: React.FC<MealFormProps> = ({
  ...
  // Recipe form modal
  const [recipeFormMode, setRecipeFormMode] = useState<{ mode: 'create' | 'edit'; recipe: Recipe | null } | null>(null);
  ...
  const loadRecipes = () => {
    recipeService.getByTrip(tripId).then(res => {
  ...
  useEffect(() => { loadRecipes(); loadIngredients(); }, [tripId]);
  ...
  return (
    <>
      {recipeFormMode && (
        <Modal
          open={!!recipeFormMode}
          onClose={() => setRecipeFormMode(null)}
          title={recipeFormMode.mode === 'create' ? 'Nouvelle recette' : 'Modifier la recette'}
        >
          <RecipeForm
            mode={recipeFormMode.mode}
            tripId={tripId}
            recipe={recipeFormMode.recipe}
            onSave={handleRecipeSaved}
            onCancel={() => setRecipeFormMode(null)}
          />
        </Modal>
      )}
      ...
      <form className="meal-form" onSubmit={handleSubmit}>
        ...
        <div className="meal-form-section">
          ...
          <div className="meal-form-checkboxes">
            {allRecipes.map(r => (
              <span key={r.id} className="form-checkbox" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5em' }}>
                <label>
                  <input type="checkbox" checked={selectedRecipeIris.includes(r['@id'])} onChange={() => toggleRecipe(r['@id'])} />
                  {r.name}
                </label>
                <button type="button" onClick={() => setRecipeFormMode({ mode: 'edit', recipe: r as any })} className="btn btn-sm btn-secondary" title="Modifier">
                  ✎
                </button>
              </span>
            ))}
            {allRecipes.length === 0 && <p className="empty-hint">Aucune recette disponible pour ce trip.</p>}
          </div>
          <button type="button" onClick={() => setRecipeFormMode({ mode: 'create', recipe: null })} className="btn btn-sm btn-secondary">
            + Nouvelle recette
          </button>
        </div>
